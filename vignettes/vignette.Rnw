...
Steps for installing and loading the REBET package: \newline 
1. Download the package from Bioconductor. \newline
 \indent if (!requireNamespace("BiocManager", quietly = TRUE)) \newline
 \indent \indent  install.packages("BiocManager") \newline
 \indent  BiocManager::install("REBET") 


\subsection{Installation of the developmental version from Github}
 devtools::install\_github("$\mbox{wheelerb/REBET}$") \newline 

\noindent 2. Load the package into an R session.
<<start>>=
library(REBET)
@
...
Read in the phenotype data containing the response, covariates, and
subject ids. 
<<pheno data>>=
...
Our model will be adjusted for age and gender. Let us add a dummy variable
for gender.
<<add dummy var>>=
data[, "MALE"] <- as.numeric(data[, "Gender"] %in% "MALE")
@

For out analysis, we are only interested in the four sub-regions of chromosome 7
defined below.
<<sub-regions>>=
...
We also need the subject order for the genotype data. These ids are in the file 
subfile. Let us read in the genotype subject ids.
<<subject file>>=
geno.subs  <- scan(subfile, what="character")
@

The set and order of subjects may not be the same in the phenotype and genotype
data. We need the common set of subject and the correct order.
<<>>=
...
The rebet function requires a vector of sub-region names for the variants
in matrix G. The sub-region names will be SR1-SR4 and will be stored in the 
vector E.
